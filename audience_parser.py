...
мы берем список найденных карточек заведений из CSV-файла location_links.csv,
оставляем только те места, у которых в yandex_categories есть 'бильярдный клуб',
открываем их страницы в Яндекс Картах через Selenium,
переходим на вкладку отзывов и собираем тексты отзывов.
...
Входной файл:
    projvenv/location_links.csv

Ожидаемые столбцы во входном файле:
    place_name         — исходное название заведения
    district           — район Москвы
    yandex_name        — название карточки в Яндекс Картах
    yandex_categories  — категории карточки в Яндекс Картах
    yandex_maps_url    — ссылка на карточку заведения в Яндекс Картах
    status             — статус результата поиска карточки
...
# Читаем CSV-файл со ссылками на карточки заведений в Яндекс Картах.
# Этот файл создается отдельным скриптом location_links.py.
places = pd.read_csv('projvenv/location_links.csv')

# Приводим основные столбцы к строкам и убираем лишние пробелы.
# Это нужно, чтобы в итоговом CSV не было случайных пробелов в названиях, категориях и ссылках.
...
# Оставляем только найденные карточки, у которых есть ссылка на Яндекс Карты.
...
# Для анализа отзывов берем только реальные бильярдные клубы.
# Так мы не тратим время на бары, кинотеатры и другие похожие места из исходного списка 2ГИС.
...
def collect_reviews_from_page(driver, place_name, district, yandex_name, yandex_categories, url):
    ...
    На вход получает:
        ...
        yandex_name       — название карточки в Яндекс Картах
        yandex_categories — категории карточки в Яндекс Картах
        url               — ссылка на карточку заведения
...
wait = WebDriverWait(driver, 10)
 ...
 # Файл, куда сохраняются собранные отзывы.
OUTPUT_PATH = Path('projvenv/audience_raw.csv')

# Здесь будут храниться все собранные отзывы по всем заведениям.
# Если файл audience_raw.csv уже существует, подгружаем его,
# чтобы при повторном запуске не начинать парсинг с нуля.
if OUTPUT_PATH.exists():
    ...
    print('Уже собрано отзывов:', len(all_reviews))
    print('Уже обработано заведений:', len(parsed_urls))
else:
    all_reviews = []
    parsed_urls = set()
...
for index, row in places.iterrows():
    ...
    if url in parsed_urls:
        print('Это заведение уже есть в audience_raw.csv, пропускаем')
        continue
    ...
    try:
        ...
        # Это полезно: если парсер упадет на следующем завдении,
